...
This repository includes a helper script for Android cross-compilation:

```bash
./build_android.sh --abi arm64-v8a
```

Build and package artifacts for Android module integration:

```bash
./build_android.sh --abi arm64-v8a --package
```

Package only from an already installed build (default prefix: `android-install/<abi>`):

```bash
./build_android.sh --abi arm64-v8a --package-only
```

Default package output layout:

```text
android-package/
  jniLibs/arm64-v8a/libessentia.so
  include/essentia/...
  pkgconfig/essentia.pc
```
